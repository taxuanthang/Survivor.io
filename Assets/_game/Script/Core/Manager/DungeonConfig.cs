...
public class DungeonConfig : ScriptableObject
{
    ...
    public Object sizeReferenceRoom;
    [Tooltip("Kéo Tile hoặc Prefab phòng (có Tilemap) vào đây.")]
    public Object sizeReferenceHall;
    ...
    public Vector2 GetActualRoomSize()
    {
        ...
        if (sizeReferenceRoom is GameObject roomPrefab)
        {
    ...
    public Vector2 GetActualHallize()
    {

        if (sizeReferenceHall is GameObject hallPrefab)
        {


            if (hallPrefab == null) return new Vector2(roomWorldSize, roomWorldSize);

            Tilemap tilemap = hallPrefab.GetComponent<Tilemap>();
            if (tilemap == null) return new Vector2(roomWorldSize, roomWorldSize);
