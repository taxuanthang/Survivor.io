...
public class CircleExpandAttack : Attack
{
    ...
    public float hitBoxDurationAfterFinishSkill = 0.5f; // thời gian tồn tại của hitbox
    ...
    public override async Task PerformAttack(Transform origin, Transform target)
    {
        ...
        currentCircleMaxScale = 0;

        hitBox.spriteRenderer.enabled = true;
        hitBox.spriteRenderer.sprite = CircleSprite;
        Vector3 createPos = origin.position;
        hitBox.SetUp(createPos, Quaternion.identity, PoolType.HitBox, 200f, innerToOuterOfHitBox);
        while (currentCircleMaxScale< circleMaxScale)
        {
            await Awaitable.NextFrameAsync();
            currentCircleMaxScale += increaseScaleSpeed*Time.deltaTime;
            hitBox.transform.localScale = new Vector3(currentCircleMaxScale, currentCircleMaxScale, 1);
        }
        hitBox.SetTimer(hitBoxDurationAfterFinishSkill);

    }

    public void ResetHitboxDame(HitBoxDame hitBox)
    {
        hitBox.spriteRenderer.enabled = false;
